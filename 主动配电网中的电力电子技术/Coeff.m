...
z = X(1:num_inequa); l = X(num_inequa+1:2*num_inequa); w = X(2*num_inequa+1:3*num_inequa);
u = X(3*num_inequa+1:4*num_inequa); x = X(4*num_inequa+1:4*num_inequa+len_x); y = X(4*num_inequa+len_x+1:4*num_inequa+len_x+num_equa);
...
%% 计算不等式约束Jacobian矩阵
dg1_dPG = eye(num_gen); dg1_dQR = zeros(num_gen,num_gen); dg1_dXtilde = zeros(2*num_node,num_gen);
dg2_dPG = zeros(num_gen,num_gen); dg2_dQR = eye(num_gen); dg2_dXtilde = zeros(2*num_node,num_gen);
dg3_dPG = zeros(num_gen,num_node); dg3_dQR = zeros(num_gen,num_node);
dg4_dPG = zeros(num_gen,num_node); dg4_dQR = zeros(num_gen,num_node);
dg3_dXtilde = zeros(2*num_PQ,num_branch); dg4_dXtilde = zeros(2*num_node,num_branch);
for ii = 1:num_node
