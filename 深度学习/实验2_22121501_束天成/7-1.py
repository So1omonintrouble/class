...
B = (0.0028 * A).sum(dim=1) + 0.028
B += torch.tensor(np.random.normal(0,0.01,size=B.size()), dtype=torch.float)
#定义数据的读取
...
class MM(nn.Module):#定义模型训练函数
    def __init__(self,num_input,num_hidden,num_output):
        ...
        self.relu = nn.ReLU()
        self.linear2 = nn.Linear(num_hidden,num_output)#定义几个神经网络层的关系
    def forward(self,x):
        x = self.linear1(x)
        x = self.relu(x)
        x = self.linear2(x)
        y = self.relu(x)#定义传递关系
        return y
...
mynet = MM(num_inputs, num_outputs, num_hiddens)
MSEloss= nn.MSELoss()

# record_k_train_ls, record_k_valid_ls = k_fold(k, X_train, y_train,if_reshape=False,num_epochs=20,num_inputs,net ,loss)
my_k_train_ls, my_k_valid_ls = k_fold(k, A, B,if_reshape=False,num_epochs=mynum_epochs,num_inputs = 500,net =mynet,loss= MSEloss)
